...
@section('body')
        ...
        <div class="tab-pane active" id="info">
            ...
            @if ($ward->exists)
                <div class="control-group">
                    {{ Form::label('change_comment', 'Close Comment', array('class' => 'control-label')) }}
                    <div class="controls">
                        {{ Form::text('change_comment', Input::old('change_comment'), array('class' => 'span11')) }}
                    </div>
                </div>
                <p>If left blank, will be set to "Closed {{$wardData->name}}"</p>
            @endif
    ...
    <div class="form-actions">
        @if ($ward->exists)
            <input type="submit" class="btn btn-primary" value="Close Ward" />
        @else
