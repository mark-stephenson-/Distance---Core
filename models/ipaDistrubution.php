...
class ipaDistrubution {
    ...
    * @param String path to where the temp files should be stored
    */
    public function __construct($ipa, $environment, $version){
		...
		$this->makeDir(dirname($ipa) . '/tmp');

		$this->getPlist($ipa);

		$this->createManifest($ipa);

		$this->seekMobileProvision($this->identiefier, $ipa);
		...
		if (is_dir($this->folder)) {
			...
			if ($zip) {
			  while ($zip_entry = zip_read($zip)) {
			    ...
			    if (($fileinfo['basename']=="Info.plist" and $fileinfo['dirname']=="Payload/NHS Prase.app") || $fileinfo['basename']==$this->itunesartwork) {
			    	$fp = fopen(dirname($ipa) . '/' . $fileinfo['basename'], "w");
    ...
    */
	function createManifest ($ipa) {
		if (file_exists(dirname(__FILE__).'/cfpropertylist/CFPropertyList.php')) {
			...
			$plist = new CFPropertyList(dirname($ipa) . '/' . 'Info.plist');
					...
					<dict>
						...
						<array>
							<dict>
								...
								<key>url</key>
								<string>'.secure_url(route('ota.download.deliver', array('ios', $this->environment, $this->version, 'app'))).'</string>
							</dict>
			...
			if (file_put_contents($this->folder."/".basename($ipa, ".ipa").".plist",$manifest)) {
                $this->applink = $this->applink.route('ota.download.deliver', array('ios', $this->environment, $this->version, 'manifest'));
            } else {
    ...
    */
	function cleanUp () {
		...
		if (file_exists(dirname($this->ipa) . "/Info.plist"))  @unlink(dirname($this->ipa) . "/Info.plist");
	}
    ...
    */
	function seekMobileProvision ($identiefier, $ipa) {
		$wildcard = pathinfo($identiefier);

		$bundels = array();
		foreach (glob(dirname($ipa) . "/*.mobileprovision") as $filename) {
